...
fn test_record_usage_accumulates_across_calls() {
    ...
    // Each record_usage invocation emits exactly one usage event; events().all()
    // returns only the current invocation's events in the soroban test env.
...
fn test_record_usage_contract_exactly_one_event_per_call() {
    ...
    // No usage events yet (before any invocation).
    assert_usage_event_count(&env, 0);

    // Single call produces exactly one event in the current invocation.
    client.record_usage(&agent, &svc, &1u32);
    assert_usage_event_count(&env, 1);

    // Second call also produces exactly one event (events().all() only
    // reflects the most recent invocation in the soroban test env).
    client.record_usage(&agent, &svc, &2u32);
    assert_usage_event_count(&env, 1);

    // Third call: again exactly one event per invocation.
    client.record_usage(&agent, &svc, &3u32);
    assert_usage_event_count(&env, 1);
}
...
fn test_record_usage_isolates_services_and_large_deltas() {
    ...
    // Assert the event immediately after record_usage, before any reads that
    // would advance the invocation context and clear events().all().
